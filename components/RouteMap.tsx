...
export const RouteMap: React.FC<RouteMapProps> = ({ points }) => {
  ...
  useEffect(() => {
    ...
    groups.append("circle")
      ...
      .attr("stroke", d => d.type === 'depot' ? '#10b981' : '#e4e4e7')
      .attr("stroke-width", 2)
  ...
  return (
    <div className="w-full bg-surface border border-border rounded-xl overflow-hidden relative">
      <div className="absolute top-4 left-4 z-10 bg-black/50 backdrop-blur px-3 py-1 rounded-md border border-border text-xs text-gray-300">
        Simulación VRP (Vehicle Routing Problem)
      </div>
      <svg ref={svgRef} className="w-full h-[400px] block" />
    </div>
  );
};